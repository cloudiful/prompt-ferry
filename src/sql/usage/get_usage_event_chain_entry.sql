SELECT rr.event_id AS "event_id!", rr.request_id AS "request_id!", rr.user_id, rr.endpoint_id, rr.path AS "path!", rr.model, rr.conversation_id, rr.parent_event_id,
       rr.conversation_seq, rr.conversation_source AS "conversation_source!", rr.client_installation_id, rr.session_header_id, rr.session_parent_id, rr.normalized_item_count,
       rr.normalized_chain_hash, rr.normalized_first_ref_hash, rr.normalized_last_ref_hash,
       rr.request_storage_mode AS "request_storage_mode!", content.request_full_json, content.request_delta_json,
       ...
       rr.provider_response_id, rr.base_checkpoint_event_id, content.response_prompt, CAST(NULL AS TEXT) AS response_raw_body
FROM request_records rr
LEFT JOIN request_record_content content
  ON content.event_id = rr.event_id
WHERE rr.event_id = $1
  AND content.event_id IS NOT NULL
