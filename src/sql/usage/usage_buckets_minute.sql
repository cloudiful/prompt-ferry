WITH bounds AS (
    SELECT date_trunc('minute', COALESCE($2::TIMESTAMPTZ, NOW() - ($1::BIGINT * INTERVAL '1 minute'))) AS start_at,
           date_trunc('minute', COALESCE($3::TIMESTAMPTZ, NOW())) AS end_at
),
buckets AS (
...
agg AS (
    ...
    WHERE event_kind = 'request'
      AND created_at >= bounds.start_at AND created_at < bounds.end_at + INTERVAL '1 minute'
