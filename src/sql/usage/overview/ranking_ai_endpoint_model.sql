SELECT
    COALESCE(pe.name, rr.endpoint_id::TEXT, '(unknown)') AS label,
    COALESCE(rr.model, '(unknown)') AS secondary_label,
    rr.endpoint_id,
    rr.model,
    rr.mcp_server_id,
...
GROUP BY rr.endpoint_id, pe.name, rr.model, rr.mcp_server_id, rr.mcp_bearer_token_slot
