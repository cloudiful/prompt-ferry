...
#[tokio::test]
async fn glm_responses_on_chat_base_fails_fast_with_remediation() {
    // Issue #238: a GLM Responses route whose base is the Chat family
    // root (`/api/coding/paas/v4`) would build a guaranteed-404 URL.
    // The forward path must surface a clear operator-facing error
    // before any upstream I/O so the operator can register a
    // dedicated Responses upstream row without chasing an opaque 404.
    let (out_tx, _bridge_log) = spawn_bridge_log();
    let services = test_services(out_tx);
    let route = glm_test_route(
    ...
    match outcome {
        ForwardOutcome::CompatError(err) => {
            ...
            assert!(
                err.message.contains("/responses"),
                "fail-fast message must mention the broken join, got: {}",
                err.message,
            );
        }
        other => panic!("expected CompatError fail-fast, got {other:?}"),
    }
}

#[tokio::test]
async fn glm_chat_and_anthropic_on_chat_base_are_untouched() {
    // The fail-fast must not bleed into the Chat or Anthropic arms on
    // the same Chat family base; only `(Glm, Responses)` is blocked.
    // The forward path reaches the URL composer and may fail on
    // protocol-mismatch validation, so the regression is pinned on
    // the error code rather than a blanket "no CompatError" — the
    // new validator returns `glm_responses_incompatible_base` and
    // pre-existing cross-protocol checks return their own codes.
    let (out_tx, _bridge_log) = spawn_bridge_log();
    let services = test_services(out_tx);
    let chat_route = glm_test_route(
        "https://open.bigmodel.cn/api/coding/paas/v4",
        NativeApi::Chat,
    );
    let outcome = forward_test_request(&services, &chat_route)
        .await
        .expect("forward");
    if let ForwardOutcome::CompatError(err) = &outcome {
        assert_ne!(
            err.code, "glm_responses_incompatible_base",
            "GLM Chat on Chat base must not trip the Responses fail-fast, got code={} message={}",
            err.code, err.message,
        );
    }

    let anthropic_route = glm_test_route(
        "https://open.bigmodel.cn/api/coding/paas/v4",
        NativeApi::AnthropicMessages,
    );
    // The shared `test_request()` always uses `/v1/responses`; the
    // Anthropic arm needs a matching path so the cross-protocol
    // validation does not pollute the regression.
    ...
    if let ForwardOutcome::CompatError(err) = &outcome {
        assert_ne!(
            err.code, "glm_responses_incompatible_base",
            "GLM Anthropic on Chat base must not trip the Responses fail-fast, got code={} message={}",
            err.code, err.message,
        );
    }
}
