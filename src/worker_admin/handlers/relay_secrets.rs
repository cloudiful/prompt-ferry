...
pub(super) fn encrypt_create_secret(
    ...
    field_name: &str,
) -> Result<Option<crate::relay_secrets::EncryptedSecretEnvelope>, Response> {
    match patch {
        None => Ok(None),
        Some(ManagedRelaySecretPatch::Replace { value }) => {
            encrypt_secret_value(manager, value, field_name)
        }
        Some(ManagedRelaySecretPatch::Keep) | Some(ManagedRelaySecretPatch::Clear) => Err(
            bad_request(&format!("{field_name} cannot use keep/clear on create")),
        ),
    }
}
...
fn encrypt_secret_value(
    ...
    if value.is_empty() {
        return Err(bad_request(&format!("{field_name} cannot be empty")));
    }
    manager
        .encrypt(&value)
        .map(Some)
