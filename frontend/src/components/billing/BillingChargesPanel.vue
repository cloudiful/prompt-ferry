...
const columns = computed<TableColumn<BillingChargeResponse>[]>(() => [
  { id: 'details' },
  ...(props.isAdmin ? [{ accessorKey: 'user_login_name', header: props.t('user') }] : []),
  { accessorKey: 'client_key_label', header: props.t('clientKey') },
  { accessorKey: 'requested_model', header: props.t('publicModel') },
  ...(props.isAdmin ? [{ accessorKey: 'endpoint_name', header: props.t('endpoint') }] : []),
...
function statusColor(charge: BillingChargeResponse): 'success' | 'warning' | 'neutral' {
  if (charge.pricing_status === 'priced' || charge.pricing_status === 'adjusted') return 'success'
...
<template>
  <section class="grid gap-3 rounded-lg bg-default p-3">
    <h2 class="text-sm font-semibold text-highlighted">{{ t('billingCharges') }}</h2>
    <div class="min-w-0 overflow-x-auto">
      <UTable :data="charges" :columns="columns" :loading="loading" class="min-w-[60rem]">
        <template #empty>{{ t('noBillingCharges') }}</template>
        <template #details-cell="{ row }">
          <UButton icon="i-lucide-receipt-text" color="neutral" variant="ghost" :aria-label="t('billingDetails')" @click="emit('openDetail', row.original)" />
        </template>
        <template #user_login_name-cell="{ row }">{{ row.original.user_login_name || '-' }}</template>
        <template #client_key_label-cell="{ row }">{{ row.original.client_key_label || '-' }}</template>
        <template #requested_model-cell="{ row }">{{ row.original.requested_model || '-' }}</template>
        <template #endpoint_name-cell="{ row }">{{ row.original.endpoint_name || '-' }}</template>
        ...
        <template #adjusted_amount-cell="{ row }">{{ formatBillingAmount(row.original.adjusted_amount, row.original.currency) }}</template>
        <template #provider_cost-cell="{ row }">{{ formatBillingAmount(row.original.provider_cost, row.original.currency) }}</template>
        <template #gross_margin-cell="{ row }">{{ formatBillingAmount(row.original.gross_margin, row.original.currency) }}</template>
        <template #created_at-cell="{ row }">{{ formatBillingTime(row.original.created_at) }}</template>
      </UTable>
    </div>
    <TablePagination :first="first" :rows="rows" :total="total" :page-size-options="BILLING_PAGE_SIZE_OPTIONS" @change="emit('page', $event)" />
  </section>
</template>
