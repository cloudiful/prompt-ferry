...
const rows = ref<ScheduleWindow[]>(cloneWindows(props.initialValue))
const initialRows = ref<ScheduleWindow[]>(cloneWindows(props.initialValue))

watch(
  () => props.initialValue,
  (value) => {
    rows.value = cloneWindows(value)
    initialRows.value = cloneWindows(value)
  },
  { deep: true },
)
...
function onSave(): void {
  // Untouched save keeps the stored schedule: emit cancel so the parent
  // keeps `active_windows_touched=false` (request omits the key).
  if (!isDirty.value) {
    emit('cancel')
    return
  }
