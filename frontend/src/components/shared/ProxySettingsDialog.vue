...
function onSave(): void {
  // Untouched save keeps the stored proxy: close without emitting so the
  // outer form keeps `proxy_url="" + has_saved=true` (request omits the key).
  ...
  <UModal v-model:open="visible" :title="t('proxySettings')">
    <template #body>
      <div class="grid gap-3 text-xs">
        ...
        <label
          v-if="scheme !== 'direct' && scheme !== 'kept'"
          class="grid gap-1"
        >
