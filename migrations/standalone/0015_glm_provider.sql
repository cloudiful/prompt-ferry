-- P1 (issue #230): mirror the PostgreSQL 0076 provider widening for the
...
-- Provider/region combination rules stay enforced by the shared admin
-- handler validation, matching the pre-existing standalone convention (the
-- column-level region CHECK is unchanged).
--
...
CREATE TABLE standalone_provider_endpoints_new (
    ...
    provider TEXT NOT NULL CHECK (provider IN ('generic', 'minimax', 'command_code', 'opencode_go', 'openrouter', 'glm')),
