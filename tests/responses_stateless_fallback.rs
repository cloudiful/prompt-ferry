...
fn orphan_tool_message_keeps_call_id_in_responses_input() {
    ...
    assert!(text.contains("call_orphan_1"), "call_id must survive, got {text}");
}
...
fn missing_tool_output_is_synthesized_with_call_id() {
    ...
    let text = String::from_utf8(out).unwrap();
    assert!(text.contains("call_missing_1"), "call_id must survive, got {text}");
    assert!(text.contains("Missing tool output"), "placeholder must be explicit, got {text}");
}
