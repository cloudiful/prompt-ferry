...
#[tokio::test]
async fn backfill_repairs_missing_text_artifacts() -> anyhow::Result<()> {
    if !test_database_configured() {
        eprintln!("skipping database integration test: {TEST_DATABASE_URL_ENV} is not set");
        return Ok(());
    }
    ...
    let dry_run =
        prompt_ferry::chat_replay::backfill_missing_assistant_artifacts(&schema.pool, false)
            .await?;
    ...
    let after_dry_run =
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM request_record_assistant_artifacts")
            .fetch_one(&schema.pool)
            .await?;
    assert_eq!(after_dry_run, 0);

    let apply =
        prompt_ferry::chat_replay::backfill_missing_assistant_artifacts(&schema.pool, true).await?;
    assert_eq!(apply.scanned, 1);
    assert_eq!(apply.repaired, 1);
