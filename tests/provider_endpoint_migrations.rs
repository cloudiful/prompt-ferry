...
// Standalone 0012 fresh path: a new store migrates to schema 12 with the
// widened provider CHECK.
#[tokio::test]
async fn standalone_0012_fresh_migration_supports_command_code() -> anyhow::Result<()> {
...
// Standalone 0012 upgrade path: a v11 database keeps its rows and gains
// command_code after open() applies the pending migration.
#[tokio::test]
async fn standalone_0012_upgrade_from_v11_preserves_rows_and_widens_provider() -> anyhow::Result<()>
{
    const APPLIED: [(i64, &str, &str); 11] = [
    ...
    insert_standalone_endpoint(&pool, "legacy-cc", "command_code", None)
        .await
        .expect_err("command_code is rejected at schema 11");
    assert_eq!(standalone_schema_version(&pool).await?, 11);

    // Record the manually applied migrations so open() only applies 0012,
    ...
    let preserved: i64 =
        sqlx::query("SELECT COUNT(*) FROM standalone_provider_endpoints WHERE name = 'legacy-minimax'")
            ...
            .await?
            .try_get(0)?;
    assert_eq!(preserved, 1, "v11 rows must survive the 0012 rebuild");
