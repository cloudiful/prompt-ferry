// Issue #384 Phase 5: approval DTOs plus the webhook result recorder moved
// from `prompt_ferry::db` into this leaf crate so `db -> llm_review` and
// `llm_review -> db` no longer form a cycle. `db` re-exports these items so
// `db::ApprovalRequest` paths are unchanged.
...
pub async fn record_approval_webhook_result(
    ...
    sqlx::query_file!(
        "src/sql/record_approval_webhook_result.sql",
        approval_id,
        error,
    )
    .execute(pool)
